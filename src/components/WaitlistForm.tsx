...
export default function WaitlistForm() {
  ...
  return (
    <div id="waitlist" className="w-full bg-white/[0.06] backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl shadow-black/30 relative max-w-4xl mx-auto">
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-500/20 blur-3xl rounded-full pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-indigo-500/20 blur-3xl rounded-full pointer-events-none" />
          ...
          {!submitted ? (
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                <div className="h-6 px-2.5 text-[10px] font-extrabold uppercase tracking-widest text-blue-300 border border-blue-400/30 bg-blue-500/10 rounded-full flex items-center gap-1.5 animate-pulse">
                  <Sparkles className="w-3 h-3 text-blue-400" />
                  Early Bird Program
                </div>
                <span className="text-xs font-bold text-blue-200/60 font-mono">Limited Spots Left</span>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-blue-100/80 block">Full Name</label>
                    <input
                      type="text"
                      required
                      ...
                      className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-blue-200/40 focus:outline-none focus:bg-white/15 focus:border-blue-400/50 focus:ring-1 focus:ring-blue-400/30 transition-all font-semibold"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-blue-100/80 block">Email Address</label>
                    <input
                      type="email"
                      required
                ...
                <div className="space-y-2.5">
                  <div className="flex justify-between items-baseline">
                    <label className="text-xs font-bold text-blue-100/80 block">
                      Which subscriptions do you currently pay for?
                    </label>
                    <span className="text-[10px] text-blue-200/50 font-mono font-bold">
                      {selectedSubs.length} Selected
                    </span>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 bg-white/5 border border-white/10 p-4 rounded-2xl max-h-[220px] overflow-y-auto">
                ...
                <button
                  type="submit"
                  className="w-full py-4 px-6 bg-gradient-to-r from-blue-500 via-indigo-500 to-blue-500 hover:from-blue-400 hover:via-indigo-400 hover:to-blue-400 text-white font-extrabold text-sm rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-500/25 cursor-pointer"
                >
            ...
            <motion.div
              ...
              className="space-y-6"
            >
              <div className="text-center p-4 bg-emerald-500/10 border border-emerald-400/30 rounded-2xl flex items-center gap-3 w-fit mx-auto">
                <Check className="w-5 h-5 text-emerald-400 bg-emerald-500/20 rounded-full p-0.5" />
                <span className="text-xs font-bold text-emerald-300">Position Secured! You are now locked in.</span>
              </div>

              {/* Glassmorphic Early Access Ticket */}
              <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-6 relative overflow-hidden shadow-lg max-w-md mx-auto">
                ...
                <div className="border-b border-dashed border-slate-350 pb-6 mb-6 flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[10px] text-blue-200/50 font-mono tracking-widest mt-1">FOUNDING MEMBER PASS</p>
                  </div>
